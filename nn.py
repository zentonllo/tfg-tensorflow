...
class DNN(object):
    ...
    def create_net(self):
        ...
        with tf.name_scope("train"):
            opt = self.optimizer(learning_rate=self.learning_rate, name='optimizer')
    ...
    def train(self, dataset, nb_epochs=100, batch_size=10, model_path=M_PATH, train_path=TR_PATH, silent_mode=False):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(nb_epochs):
                ...
                acc_train = self.accuracy.eval(feed_dict={self.is_training: False, self.X: x_training, self.y: y_training})
        ...
        self.print_time_training(start_time, time.time())
        if silent_mode:
            print("AUC:", best_auc)
    ...
    def test(self, x_test, y_test, model_path=M_PATH):
        with tf.Session() as sess:
            self.saver.restore(sess, model_path)
            acc_test = self.accuracy.eval(feed_dict={self.is_training: False, self.X: x_test, self.y: y_test})
...
if __name__ == "__main__":
    ...
    dnn.print_time_training(start_validation, time.time())
