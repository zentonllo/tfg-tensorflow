...
gcloud ml-engine jobs submit training "$JOB_ID" --stream-logs --runtime-version 1.2 --job-dir $GCS_JOB_DIR --module-name trainer.task --package-path trainer/ --scale-tier BASIC_GPU --region us-central1 -- --train-files $TRAIN_FILE --eval-files $EVAL_FILE --train-steps $TRAIN_STEPS --first-layer-size 3 --num-layers 1 > log.txt
